...
const Messages = ({ navigation }) => {
  ...
  const getUsers = async () => {
    ...
    setemail(email)
    const collectionRef = firestore().collection(email);
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={{ fontSize: 30, color: 'black' }}>Chats</Text>

   {People==''?<Text style={{fontSize:20,color:'black'}}>No chat found...</Text>:<FlatList
        data={People}
        renderItem={({ item }) => (
